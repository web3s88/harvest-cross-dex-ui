...
export default function PoolFinder() {
  ...
  const prerequisiteMessage = (
    <div className="p-5 text-center rounded bg-whitesmoke">{i18n._(t`Select a token to find your liquidity`)}</div>
  )

  return (
    <Container id="find-pool-page" className="py-4 space-y-6 md:py-8 lg:py-12" maxWidth="2xl">
      <Head>
        <title>{i18n._(t`Find Pool`)} | Harvest</title>
      ...
      <Alert
        ...
        type="information"
      />
      <div className="relative p-4 space-y-4 rounded bg-white shadow-liquidity">
        <AutoColumn gap={'md'}>
          <CurrencySelectPanel
            currency={currency0}
          ...
          <AutoColumn justify="space-between">
            <AutoRow justify={'flex-start'} style={{ padding: '0 1rem' }}>
              <button className="z-10 -mt-6 -mb-6 rounded-full bg-white p-3px">
                <div className="p-3 rounded-full bg-whitesmoke hover:bg-opacity-40">
                  <Plus size="32" />
                </div>
              </button>
            </AutoRow>
        ...
        {currency0 && currency1 ? (
          pairState === PairState.EXISTS ? (
            ...
            <div className="p-5 text-center rounded bg-whitesmoke">{i18n._(t`Invalid pair`)}</div>
          ) : pairState === PairState.LOADING ? (
            <div className="p-5 text-center rounded bg-whitesmoke">

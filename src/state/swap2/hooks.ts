...
export function useSwapState2(): AppState['swap2'] {
  return useAppSelector((state) => state.swap2)
}

export function useSwapActionHandlers2(): {
  onCurrencySelection2: (field: Field, currency: Currency, force: boolean) => void
  onSwitchTokens: () => void
  onUserInput2: (field: Field, typedValue: string) => void
  onChangeRecipient: (recipient: string | null) => void
} {
  const dispatch = useAppDispatch()
  const onCurrencySelection2 = useCallback(
    (field: Field, currency: Currency, force: boolean = false) => {
      dispatch(
        selectCurrency({
          field,
...
export function useDerivedSwapInfo2(
  doArcher = false,
  singlePair = false,
  customId: number
): {
  ...
  const { account, chainId, library } = useActiveWeb3React(customId)

  const [singleHop] = useUserSingleHopOnly()

  const singleHopOnly = singlePair || singleHop
...
export function queryParametersToSwapState(parsedQs: ParsedQs, chainId: ChainId = ChainId.MAINNET): SwapState {
  ...
  const usd = STABLE_USD[chainId].address
  if (inputCurrency === '' && outputCurrency === '') {
    inputCurrency = usd
    outputCurrency = eth
  } else if (inputCurrency === '') {
    inputCurrency = outputCurrency === eth ? usd : eth
  } else if (outputCurrency === '' || inputCurrency === outputCurrency) {
    outputCurrency = inputCurrency === eth ? usd : eth
  }
...
export function useDefaultsFromURLSearch2(chainId: number):
  | {

...
import CROSS_BRIGE_ABI from '../constants/abis/cross-bridge.json'
import TOKEN_BRIGE_ABI from '../constants/abis/token-bridge.json'
...
// CrossSwap
const TopBridgeAddress: {[p: number]: string} = {
  1: '0xd844C69ed51EA9842A4974BEa5D41901c76620fd',
  56: '0x68cBe6A76b8bF607E8160eAd1b20fda67cfB1d00',
  137: '0x8153858774E549D5d702818817f5f4dD0987452e'
}

const TokenBridgeAddress: {[p: number]: string} = {
  1: '0x884Edd7637165c24149aEB91AabB71e25D2a1EDf',
  56: '0xb1BC34d96d50C7CA06AA0EA83A634B1F7Be2ed3D',
  137: '0x0a4111D23d9DE6A94580e676c9ef806BEde7EC67'
}
...
export function useContract(
  ...
  const { chainId: activeId, library, account } = useActiveWeb3React()

  return useMemo(() => {
    if (chainId && chainId !== activeId) {
      if (!address || !ABI) return null
      try {
        return getContract(address, ABI, new Web3Provider(getNetwork(chainId).provider.request))
      } catch (error) {
        console.error('Failed to get contract for custom chain', error)
      }
    }
...
export function useTokenContract(tokenAddress?: string, withSignerIfPossible?: boolean, chainId?: number): Contract | null {
...
export function useMulticall2Contract(customId?: number) {
  const { chainId } = useActiveWeb3React(customId)
  return useContract(chainId && MULTICALL2_ADDRESS[chainId], MULTICALL2_ABI, false, chainId)
}
...
export function useExternalRouterContract(chainId: number): Contract | null {
  const address = ROUTER_ADDRESS[chainId]
  const abi = ROUTER_ABI
  return useContract(address, abi, false, chainId)
}
...
export function useCrossBridgeContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(TopBridgeAddress[chainId], CROSS_BRIGE_ABI, withSignerIfPossible)
}

export function useTokenBridgeContract(withSignerIfPossible?: boolean): Contract | null {
  const { chainId } = useActiveWeb3React()
  return useContract(TokenBridgeAddress[chainId], TOKEN_BRIGE_ABI, withSignerIfPossible)
}

import { ChainId, Currency, CurrencyAmount, Price, Token, USD, USDC } from '@sushiswap/sdk'

import { useActiveWeb3React } from '../hooks/useActiveWeb3React'
import { useMemo } from 'react'
import { useV2TradeExactOut } from './useV2Trades'
import { BSC } from '../config/tokens'
...
const STABLECOIN_AMOUNT_OUT: { [chainId: number]: CurrencyAmount<Token> } = {
  [ChainId.MAINNET]: CurrencyAmount.fromRawAmount(USDC[ChainId.MAINNET], 1e6),
  ...
  [ChainId.MATIC]: CurrencyAmount.fromRawAmount(USDC[ChainId.MATIC], 1e6),
  [ChainId.FANTOM]: CurrencyAmount.fromRawAmount(USDC[ChainId.FANTOM], 100_000e6),
  [ChainId.BSC]: CurrencyAmount.fromRawAmount(BSC.USD, 1e18),
...
export default function useUSDCPrice(currency?: Currency): Price<Currency, Token> | undefined {
  const { chainId } = useActiveWeb3React(currency?.chainId)
...
export function useUSDCValue(currencyAmount: CurrencyAmount<Currency> | undefined | null) {
  ...
  // console.log(price, currencyAmount?.currency)
